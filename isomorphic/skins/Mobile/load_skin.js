...
isc.loadSkin = function (theWindow) {
    ...
    with (theWindow) {
        ...
        if (isc.TabSet) {
            isc.defineClass("MobileTabButton", "Button").addProperties({
                setIcon : function (icon) {
                    // Make sure a previous icon is replaced
                    this._mobileIcon = null;
                    this.Super("setIcon", arguments);
                },
                getTitle : function () {
                    if (!isc.Browser.isWebKit) return this.Super("getTitle", arguments);

                    if (!this._mobileIcon && this.icon) {
                        this._mobileIcon = this.icon;
                        this.icon = null;
                    }
                    var imgHTML = (this._mobileIcon == null
                        ? "<span style='height: 30px'>&nbsp;</span>"
                        : isc.Canvas.imgHTML("[SKIN]blank.gif", 30, 30, null, 
                              "style='-webkit-mask-box-image: url(" +
                                    isc.Page.getImgURL(this._mobileIcon) + 
                              ");",
                              null, null)),
            ...
            isc.TabSet.addProperties({
                useSimpleTabs:true,
                simpleTabButtonConstructor:isc.MobileTabButton,
